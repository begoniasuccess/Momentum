...
def fliterColsePirceByMonth(sDt: datetime, eDt: datetime, filterMeanClose: int = 0) -> bool:
    result = True
    try:
        # 資料夾設定
        target_folder = Path(storageDir_summary + f"/closePrice/filterByAdjMean/over{filterMeanClose}")
        target_folder.mkdir(parents=True, exist_ok=True)

        src_folder = Path(storageDir_summary + "/closePrice")
        mean_folder = Path(storageDir_summary + "/adjPriceMeanByMonth")

        # 遍歷時間範圍
        curDt = sDt
        while curDt <= eDt:
            year = curDt.year
            yyyymm = curDt.strftime("%Y%m")

            # 輸出檔案
            target_file = target_folder / f"closePrice_{yyyymm}.csv"
            if target_file.exists():
                utils.ptMsg(f"✅ 已存在 {target_file.name}，跳過")
                curDt += relativedelta(months=1)
                continue

            # 載入該年份的篩選檔案
            mean_file = mean_folder / f"{year}_meanOver{filterMeanClose}.csv"
            if not mean_file.exists():
                utils.ptMsg(f"⚠️ 找不到篩選檔 {mean_file.name}，跳過")
                curDt += relativedelta(months=1)
                continue

            mean_df = pd.read_csv(mean_file, dtype={'year_month': str, 'stock_id': str})
            month_filter_df = mean_df[mean_df['year_month'] == yyyymm]
            filter_stock_ids = set(month_filter_df['stock_id'])

            if not filter_stock_ids:
                utils.ptMsg(f"⚠️ {yyyymm} 沒有符合條件的股票，跳過")
                curDt += relativedelta(months=1)
                continue

            # 載入該月份的closePrice檔案
            src_file = src_folder / f"closePrice_{yyyymm}.csv"
            if not src_file.exists():
                utils.ptMsg(f"⚠️ 找不到原始檔 {src_file.name}，跳過")
                curDt += relativedelta(months=1)
                continue

            close_df = pd.read_csv(src_file, dtype={'stock_id': str})
            close_df_filtered = close_df[close_df['stock_id'].isin(filter_stock_ids)]

            if close_df_filtered.empty:
                utils.ptMsg(f"⚠️ {yyyymm} 篩選後無資料，跳過")
                curDt += relativedelta(months=1)
                continue

            # 儲存篩選後的檔案
            close_df_filtered.to_csv(target_file, index=False)
            utils.ptMsg(f"📁 已產出：{target_file.name} ({len(close_df_filtered):,} 筆)")

            curDt += relativedelta(months=1)

    except Exception as e:
        utils.ptMsg(f"⚠️ anaData.fliterColsePirceByMonth()失敗，錯誤：{e}")
        return False
