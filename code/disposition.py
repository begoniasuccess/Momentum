...
dispDir = f"../data/TwStockExchange/DispositionStock"
# file_path = f"{dispDir}/{sDt.strftime('%Y%m%d')}_{eDt.strftime('%Y%m%d')}-simple.csv"
# df_disp = pd.read_csv(file_path, parse_dates=["公布日期", "處置起始", "處置結束"])

table = "tw_stock_daily_price_adj"

outputFile = "test.csv"
df_disp = pd.read_csv(outputFile, parse_dates=["公布日期", "處置起始", "處置結束"])
for year in range(sDt.year, eDt.year + 1):
    ...
    # 每月存檔一次
    df_disp.to_csv(outputFile, index=False)
