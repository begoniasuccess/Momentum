...
hPeriods = [3] # Holding Period

for oPeriod, hPeriod in itertools.product(oPeriods, hPeriods):
    ...
    if not dataExist:
        ...
        close_df = None
        close_df_year = "0"
        closeDfYears = (oPeriod + hPeriod)//12 + 1
    ...
    ### 計算持有期的報酬
    filePrefixIdx = filePrefixIdx + 1
    output_file = Path(getOutputCsvPath(target_folder, filePrefixIdx, "afterwardReturn"))
    if os.path.exists(output_file):
        filtered_df = pd.read_csv(output_file)
        utils.ptMsg(f"☑️ 檔案已存在：{output_file}")
    else:
    ...
    ### 統計持有期間平均報酬
    filePrefixIdx = filePrefixIdx + 1
    output_file = Path(getOutputCsvPath(target_folder, filePrefixIdx, "afterwardReturn_static"))
    if os.path.exists(output_file):
    ...
    ### 計算winner - loser
    filePrefixIdx = filePrefixIdx + 1
    output_file = Path(getOutputCsvPath(target_folder, filePrefixIdx, "afterwardReturn_static2"))
    if os.path.exists(output_file):
