...
dataExist = False
# 路徑設定
target_folder = r'..\data\analysis\momentumNew' + f'/oPeriod{oPeriod}_hPeriod{hPeriod}'
output_file = f'{target_folder}/observerReturnList{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}.csv'
if os.path.exists(output_file):
...
if not dataExist:
    ...
    while cur_dt <= eDt:
        ...
        if (close_df is None) or (int(close_df_year) != int(curY)):            
            close_df = utils.getCloseDf(curY, 2)
...
### 增加各種rank相關欄位
output_file = f"{target_folder}/observerReturnList{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}-rank.csv"
if os.path.exists(output_file):
    result_df = pd.read_csv(output_file)
    utils.ptMsg(f"☑️ 檔案已存在：{output_file}")
else:
...
### 產生winner_loser名單
output_file =  f"{target_folder}/winner_loser-{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}.csv"
if os.path.exists(output_file):
...
### 計算持有期的報酬
output_file = Path(f"{target_folder}/afterwardReturn-{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}.csv")
if os.path.exists(output_file):
    filtered_df = pd.read_csv(output_file)
    utils.ptMsg(f"☑️ 檔案已存在：{output_file}")
else:
    ...
    # print("start_date2_list", start_date2_list)
    # print("end_date2_list", end_date2_list)
    # print("ED_close2_list", ED_close2_list)
    # print("ED_close2_list", ED_close2_list)
...
### 統計持有期間平均報酬
output_file = Path(f"{target_folder}/afterwardReturn-{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}-static.csv")
if os.path.exists(output_file):
...
### 計算winner - loser
output_file = Path(f"{target_folder}/afterwardReturn-{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}-static2.csv")
if os.path.exists(output_file):
...
### t-test
output_file = Path(f"{target_folder}/t_test_results-{sDt.strftime("%Y%m")}_{eDt.strftime("%Y%m")}.csv")
if os.path.exists(output_file):
    utils.ptMsg(f"☑️ 檔案已存在：{output_file}")
else:
    # 移除多餘逗號
